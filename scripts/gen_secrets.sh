...
# - Common (optional): keycloak-credentials in mip-common-datacatalog
...
# -------------------------- common keycloak-credentials -----------------------
if secret_exists "mip-common-datacatalog" "keycloak-credentials" && [[ "$FORCE_UPDATE" -eq 0 ]]; then
  echo "[skip] mip-common-datacatalog/keycloak-credentials exists."
else
  # Allow env to pre-seed; otherwise, prompt
  ...
  if [[ -z "${COMMON_KC_CLIENT_ID:-}" || -z "${COMMON_KC_CLIENT_SECRET:-}" ]]; then
    echo "[err] missing common client-id/secret; aborting." >&2
    exit 1
  fi
...
type: Opaque
stringData:
  client-id: ${COMMON_KC_CLIENT_ID}
  client-secret: ${COMMON_KC_CLIENT_SECRET}
EOF
  if apply_yaml_file "mip-common-datacatalog" "$f"; then
    echo "[ok ] mip-common-datacatalog/keycloak-credentials"
  else
    echo "[err] mip-common-datacatalog/keycloak-credentials" >&2
  fi
  secure_rm "$f"
